...
public partial class admin_tpxw : System.Web.UI.Page
{
    ...
    private void company(DropDownList ddlname)
    {
        ...
        ddlname.DataValueField = "ID";
        ddlname.DataBind();
        ddlname.Items.Insert(0, new ListItem("==选择企业==", "0"));
        ddlname.SelectedValue = "";
    }
    ...
    protected void bt_Click(object sender, EventArgs e)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            try
            {
                ...
                DropDownList1.SelectedValue = dr["companyid"].ToString();
    ...
    protected void bc_Click(object sender, EventArgs e)
    {
        ...
        if (id.Text.Equals("0"))
        {
            ...
            sql = "insert into zqhl_pic(title,link,pic,en,qz,typeid,companyid)values(";
            sql += "'" + Common.strFilter(title.Text) + "','" + Common.strFilter(link.Text) + "',";
            sql += "'" + Common.strFilter(pic.Text) + "'";//,'admin')";
            sql += "," + (en.Checked ? "1" : "0") + "," + Common.strFilter(qz.Text) + "," + ddlWType.SelectedValue + "," + DropDownList1.SelectedValue + ")";
        }
        else
        {
            ...
            sql += ",en=" + (en.Checked ? "1" : "0") + ",qz=" + Common.strFilter(qz.Text)+ ",typeid=" + ddlWType.SelectedValue + ",companyid=" + DropDownList1.SelectedValue + "";
