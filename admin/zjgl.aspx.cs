...
public partial class admin_zjgl : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        DataTable dt;
        dt = DBC.getDataTable(@"select (select sum(1) from zqhl_zj where id<=b.id) as IID,*,fl=(select top 1 [class] from zqhl_class a where a.id=b.classid),rname=(select RealName from [ZhongQiHuiLian2018].dbo.[user] u  where u.userid=b.userid) from zqhl_zj b where " + tj + " order by b.id desc");
        try
        {
