...
using System.Data;
public partial class admin_zxsq : System.Web.UI.Page
{
    ...
    {
        string tj = " qymc like '%" + Common.strFilter(title.Text) + "%'";
        DataTable dt;
        dt = DBC.getDataTable("select * from zqhl_zxsq b where " + tj + " order by b.id desc");
        try
        {
    ...
    {
        Response.Redirect("zx.aspx?id=" + e.CommandArgument);
    ...
    {
        DBC.getRowsCount("delete from zqhl_zxsq where id=" + e.CommandArgument);
        BindGrid();
    }

    protected void new_Click(object sender, EventArgs e)
    {
        Response.Redirect("zx.aspx?id=0");
    }
