...
public partial class zxsq : System.Web.UI.Page
{
    ...
    protected void bc_Click(object sender, EventArgs e)
    {
        msg.Text = "";
        string sql = "";
        if (qymc.Text.Length == 0)
        {
            msg.Text = "企业名称不能为空"; return;
        }
        if (lxr.Text.Length == 0) { msg.Text = "您的姓名不能为空"; return; }
        if (lxdh.Text.Length == 0) { msg.Text = "联系电话不能为空"; return; }
        if (qymc.Text.Length == 0)
        {
            msg.Text = "企业名称不能为空"; return;
        }
        sql = "insert into zqhl_zxsq(qymc,lxr,lxdh,xmmc,xmjs,jsys,cqqk,tzjd,xslr)values(";
        sql += "'" + Common.strFilter(qymc.Text) + "','" + Common.strFilter(lxr.Text) + "','" + Common.strFilter(lxdh.Text) + "','" + Common.strFilter(xmmc.Text) + "','" + Common.strFilter(xmjs.Text) + "'";
        sql += ",'" + Common.strFilter(jsys.Text) + "','" + Common.strFilter(cqqk.Text) + "','" + Common.strFilter(tzjd.Text) + "','" + Common.strFilter(xslr.Text) + "')";
        int count = DBC.getRowsCount(sql);
        if (count > 0) { msg.Text = "提交成功，请等待联系"; } else { msg.Text = "提交失败"; }
    }
}