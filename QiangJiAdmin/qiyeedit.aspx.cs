...
public partial class admin_qiyeedit : System.Web.UI.Page
{
    ...
    protected void Button7_Click(object sender, EventArgs e)
    {
        ...
        //hezuozt.Text = dr["KeyAreas"].ToString();
        string sql = @"UPDATE [dbo].[Company] SET   [logo] = '" + Common.strFilter(pic.Text) + "',[lianmengtype] = '" + Common.strFilter(ddlLianMengType.SelectedValue) + "',[typeid] = '" + Common.strFilter(cbtext1) + "' ,[url] = '" + Common.strFilter(url.Text) + "'    WHERE ID=" + sid + "";
        int count = DBqiye.getRowsCount(sql);
        typeid = 7;
